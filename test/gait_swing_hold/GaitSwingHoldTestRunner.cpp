...
GaitSwingHoldTestRunner::GaitSwingHoldTestRunner(const RobotType robotType,
    ...
    if (_controller == nullptr) {
        throw std::invalid_argument("GaitSwingHoldTestRunner requires a controller");
    }
    _controller->setTouchdownTargetZOffset(_torsoZOffset);
}

GaitSwingHoldTestRunner::~GaitSwingHoldTestRunner() {
    if (_data != nullptr) {
        mj_deleteData(_data);
        _data = nullptr;
    }
...
void GaitSwingHoldTestRunner::init() {
...
void GaitSwingHoldTestRunner::run() {
    if (_headless) {
    ...
    std::thread physicsThread(&GaitSwingHoldTestRunner::runPhysicsLoop, this, true, true);
    _mainThread.run();
...
void GaitSwingHoldTestRunner::runPhysicsLoop(const bool throttleRealtime, const bool syncViewer) {
...
void GaitSwingHoldTestRunner::initializeControllerRuntime() {
    ...
    if (_legQposIndices.empty() || _legQvelIndices.empty()) {
        throw std::runtime_error("GaitSwingHoldTestRunner could not find leg qpos indices");
    }
...
void GaitSwingHoldTestRunner::runRobotControl() {
...
void GaitSwingHoldTestRunner::applyRobotCommand() {
...
void GaitSwingHoldTestRunner::updateDebugVisualization() {
...
void GaitSwingHoldTestRunner::locateFloatingBase() {
    ...
    if (_freeJointQposIndex < 0 || _freeJointQvelIndex < 0) {
        throw std::runtime_error("GaitSwingHoldTestRunner could not find a floating base joint");
    }
}

void GaitSwingHoldTestRunner::cacheFrozenQpos() {
...
bool GaitSwingHoldTestRunner::isLegQposIndex(const int qposIndex) const {
    return std::find(_legQposIndices.begin(), _legQposIndices.end(), qposIndex) !=
           _legQposIndices.end();
}

bool GaitSwingHoldTestRunner::isLegQvelIndex(const int qvelIndex) const {
    return std::find(_legQvelIndices.begin(), _legQvelIndices.end(), qvelIndex) !=
           _legQvelIndices.end();
}

void GaitSwingHoldTestRunner::clampFrozenQpos() {
